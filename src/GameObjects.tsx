...
export function GameObjects (props: GameObjectsProps) {
    const currentLevel = useRef({id: props.state.fieldInfo.fieldID, step: props.step});
    const viewState = useRef(initViewState(props));
    if (currentLevel.current.id !== props.state.fieldInfo.fieldID ||
        currentLevel.current.step !== props.step) {
        viewState.current = initViewState(props);
        currentLevel.current = {id: props.state.fieldInfo.fieldID, step: props.step};
    }
        ...
        <div className="sokoban-objects">
            {viewState.current.viewOrder.map(item => {
                switch(item.type) {
                    case "box":
                        return (
                            <BoxObject key={item.id} x={item.x} y={item.y}
                            placed={item.placed} scale={props.step} onClick={props.boxHandler} />
                        )
                        break;
                    case "worker":
                        return (
                            <WorkerObject key={-1} x={item.x} y={item.y}
                            direction = {item.direction} scale={props.step} onClick={props.workerHandler}/>
                        )
                        break;
                }
            })}
...
function initViewState(props: GameObjectsProps) {
    const maxSize = Math.max (props.state.fieldInfo.heigth, props.state.fieldInfo.width);
    let workerIndex = 0;
    const view = props.state.field.reduce((acc:Array<ObjectItem>, item, index)=>{
...
function applyChanges(props: GameObjectsProps, viewState: ViewState) {
    if (props.state.lastMove.worker === null ||
        props.state.workerIndex === viewState.previousStateIndex) {
        return;
    }
    ...
    ([worker.x, worker.y] = newCoordinates(props.state.lastMove.worker, props.step, worker));
    if (props.state.lastMove.box !== null) {
        const box = viewState.orderByID[props.state.lastMove.box];
        ([box.x, box.y] = newCoordinates(props.state.lastMove.worker, props.step, box));
    }
...
function moveObjects(lastMove: MoveResult, viewState: ViewState, step: MoveStep) {
    ...
    if (lastMove.box !== null) {
        ...
        while (boxIndex !== lastIndex && viewState.viewOrder[boxIndex].id !== lastMove.box) 
            boxIndex += step;
        localSort(viewState.viewOrder, boxIndex, step);
    }
    viewState.viewWorkerIndex = localSort(viewState.viewOrder, viewState.viewWorkerIndex, step);
}
...
function newCoordinates(direction: Direction, step: number, object: ObjectItem) {
    const stepX = step * 2;
    let x = object.x;
    let y = object.y;
    switch (direction) {
